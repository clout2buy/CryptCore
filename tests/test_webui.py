...
def test_webui_snapshot_endpoint(monkeypatch, tmp_path: Path):
    ...
    try:
        ...
        assert "businessCrm" in web_snapshot
        ...
        assert "accessibilityMotionAudit" in web_snapshot
...
def test_webui_static_is_chat_first():
    ...
    assert "Accessibility Motion Audit" in script
    assert "accessibilityMotionAudit" in script
    ...
    assert "WebUI Cache" in script
    assert "webuiCacheHealth" in script
    ...
    assert "External Receipts" in script
    assert "externalReceipts" in script
    ...
    assert "Business CRM" in script
    assert "businessCrm" in script
